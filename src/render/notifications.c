...
/*   Updated: 2021/01/07 11:48:55 by ohakola          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					int32_t	dims[2], int32_t border_size)
{
	...
	y = pos[1] - 1;
	while (++y < (int32_t)pos[1] + (int32_t)dims[1])
	{
		x = pos[0] - 1;
		while (++x < (int32_t)pos[0] + (int32_t)dims[0])
		{
				...
				x >= 0 && x < app->window->framebuffer->width)
			{
				app->window->framebuffer->buffer[
					y * app->window->framebuffer->width + x] =
						CLEAR_COLOR;
				if (x >= (int32_t)pos[0] + (int32_t)dims[0] - border_size ||
					x < (int32_t)pos[0] + border_size ||
					y >= (int32_t)pos[1] + (int32_t)dims[1] - border_size ||
					...
					int32_t	text_dims[2], int32_t padding)
{
	...
	while (++i < (int32_t)app->notifications.num_notifications)
	{
		window_text_render(app->window, (t_text_params){
			...
			.xy = (int32_t[2]){pos[0] + padding, pos[1] + i * text_dims[1]}
...
void			notifications_render(t_doom3d *app, t_vec2 pos)
{
	...
	while ((int32_t)pos[1] + dims[1] < 0)
		pos[1]++;
	while ((int32_t)pos[0] + dims[0] > app->window->framebuffer->width)
		pos[0]--;
