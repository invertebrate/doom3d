														...
														int32_t x, int32_t y)
{
	...
	error_check(!(sub_buffer = ft_memalloc(sizeof(t_sub_framebuffer))),
	...
	sub_buffer->y_offset = sub_buffer->parent_height * 0.5 -
		sub_buffer->y_start;
	error_check(!(
		sub_buffer->buffer = ft_memalloc(sizeof(uint32_t) *
			sub_buffer->height * sub_buffer->width)),
			"Failed to malloc frame sub buffer");
	error_check(!(
		sub_buffer->zbuffer = ft_memalloc(sizeof(float) *
			sub_buffer->height * sub_buffer->width)),
			"Failed to malloc frame sub buffer");
	return (sub_buffer);
}
								...
								int32_t height)
{
	...
	error_check(!(fbuffer = ft_memalloc(sizeof(t_framebuffer))), "!ft_memallocf");
	error_check(!(fbuffer->buffer =
		ft_memalloc(sizeof(uint32_t) * width * height)), "Failed to malloc fbuffer");
	fbuffer->num_x = L3D_BUFFER_SPLIT_SIZE_X;
	fbuffer->num_y = L3D_BUFFER_SPLIT_SIZE_Y;
	fbuffer->width = width;
	fbuffer->height = height;
	error_check(!(fbuffer->sub_buffers = ft_memalloc(sizeof(t_sub_framebuffer*) *
