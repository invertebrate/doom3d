...
/*   Updated: 2021/05/01 22:54:10 by ohakola          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
				...
				int32_t offset_xy[2], uint32_t pixel)
{
	...
	under = l3d_pixel_get(buffers->buffer,
			(uint32_t[2]){buffers->width, buffers->height}, offset_xy);
	return (l3d_color_alpha_blend_u32(under, pixel));
}
